...
export async function POST(req: Request) {
  try {
    ...
    switch (style) {
      case "realistic":
        stylePrompt = "이 사진은 간단한 낙서를 카메라로 찍은 사진입니다. 간단한 낙서를 매우 상세하고 현실세계의 이미지로 변환해주세요.종이안의 낙서를 분석해서 실제 오브젝트로 처리해야합니다. 종이를 잡은 손가락등 외부 환경은 제외해야함 "
        break
      case "cartoon":
        stylePrompt = "이 낙서를 부드러운 선과 밝은 색상, 매력적인 캐릭터 디자인을 가진 생동감 넘치고 귀여운 만화 일러스트로 변환해주세요. 표현력 있는 특징을 추가하고 전문 애니메이션 스틸처럼 보이게 만들어주세요.종이안의 낙서를 분석해서 실제 오브젝트로 처리해야합니다. 종이를 잡은 손가락등 외부 환경은 제외해야함"
        break
      case "3d":
        stylePrompt = "이 낙서를 사실적인 재질과 적절한 조명, 그림자, 깊이감을 가진 고품질 3D 렌더링 모델로 변환해주세요. 매끄러운 표면과 정확한 비율을 가진 전문적인 3D 시각화처럼 보이게 만들어주세요. 종이안의 낙서를 분석해서 실제 오브젝트로 처리해야합니다. 종이를 잡은 손가락등 외부 환경은 제외해야함"
        break
      case "painting":
        stylePrompt = "이 낙서를 보이는 붓터치와 풍부한 색상 팔레트, 예술적인 구성을 가진 아름다운 유화로 변환해주세요. 회화적 스타일로 원래 개념을 유지하면서 질감과 깊이를 추가해주세요. 종이안의 낙서를 분석해서 실제 오브젝트로 처리해야합니다. 종이를 잡은 손가락등 외부 환경은 제외해야함"
        break
      case "digital-art":
        stylePrompt = "이 낙서를 깔끔한 선과 생동감 있는 색상, 전문적인 마감을 가진 현대적인 디지털 아트워크로 변환해주세요. 부드러운 그라데이션과 세련된 외관을 가진 현대적 디지털 아트 기법을 사용해주세요. 종이안의 낙서를 분석해서 실제 오브젝트로 처리해야합니다. 종이를 잡은 손가락등 외부 환경은 제외해야함"
        break
      case "sketch":
        stylePrompt = "이 낙서를 적절한 음영과 질감, 예술적 기법을 가진 세련되고 상세한 연필 스케치로 변환해주세요. 깔끔한 선과 깊이감을 가진 전문적인 일러스트레이션처럼 보이게 만들어주세요. 종이안의 낙서를 분석해서 실제 오브젝트로 처리해야합니다. 종이를 잡은 손가락등 외부 환경은 제외해야함"
        break
      default:
