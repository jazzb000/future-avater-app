...
export async function POST(req: Request) {
  try {
    ...
    switch (style) {
      case "realistic":
        stylePrompt = "이 사진은 간단한 낙서를 카메라로 찍은 사진입니다. 간단한 낙서를 매우 상세하고 사진처럼 사실적인 이미지로 변환해주세요. 하얀색 종이안의 낙서만을 분석해서 처리해야합니다. 핵심 개념과 구성을 유지하면서 사실적인 질감, 조명, 그림자, 세밀한 디테일을 추가해주세요."
        break
      case "cartoon":
